...
int main()
{
    ...
    Mat A2(8, 8, CV_32F, Scalar::all(0));
    A.copyTo(A2(Rect(0, 0, A.cols, A.rows)));
    cout<<"A2 = "<<A2<<endl;

    ChangeSignOddPositionInXY(A2); //centering
    Mat dftA;
    dft(A2, dftA, DFT_COMPLEX_OUTPUT);
    ...
    Printmat("dftB(Rect(0, 0, A.cols, A.rows)) = ", dftB(Rect(0, 0, A.cols, A.rows)));
